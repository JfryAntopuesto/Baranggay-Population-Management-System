...
try {
    ...
    $check_table = "SHOW TABLES LIKE 'approved_tickets'";
    $table_result = $conn->query($check_table);
    
    if ($table_result->num_rows === 0) {
        // Table doesn't exist, create it
        $create_table = "CREATE TABLE approved_tickets (
        ...
        if (!$conn->query($create_table)) {
            throw new Exception("Failed to create approved_tickets table: " . $conn->error);
        }
    }

    $sql = "SELECT t.*, u.firstname, u.middlename, u.lastname 
            FROM approved_tickets t 
