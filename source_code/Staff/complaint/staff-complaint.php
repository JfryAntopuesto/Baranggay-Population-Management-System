    ...
    <div class="container">
        <div class="left-panel">
            <div class="sidebar-nav">
                ...
                <a href="staff-complaint.php" class="sidebar-btn active">COMPLAINT <span class="badge" id="pendingComplaintsBadge">0</span></a>
    ...
    <script>
        ...
        function renderComplaints() {
            ...
            complaints.forEach((complaint, idx) => {
                ...
                card.innerHTML = `
                    ...
                    <label>DATE:</label> <span>${complaint.created_at || 'N/A'}</span>
                `;
                card.onclick = () => openComplaintModal(idx);
                pendingDiv.appendChild(card);
            });

            // Display approved complaints in the FINISHED column
            if (approvedComplaints && approvedComplaints.length > 0) {
                approvedComplaints.forEach((complaint) => {
                    const card = document.createElement('div');
                    card.className = 'request-card';
                    card.innerHTML = `
                        <span class="sender">${(complaint.firstname || '') + ' ' + (complaint.middlename || '') + ' ' + (complaint.lastname || '')}</span>
                        <label>COMPLAINT #:</label> <span>${complaint.complaintID}</span><br>
                        <label>TYPE:</label> <span>${complaint.type ? complaint.type.toUpperCase() : 'N/A'}</span><br>
                        <label>MESSAGE:</label> <span>${complaint.message || 'N/A'}</span><br>
                        <label>DATE APPROVED:</label> <span>${complaint.created_at || 'N/A'}</span>
                    `;
                    finishedDiv.appendChild(card);
                });
            }

            // Display declined complaints in the DECLINED column
            if (declinedComplaints && declinedComplaints.length > 0) {
                declinedComplaints.forEach((complaint) => {
                    ...
                    card.innerHTML = `
                        ...
                        <label>TYPE:</label> <span>${complaint.type ? complaint.type.toUpperCase() : 'N/A'}</span><br>
                        <label>MESSAGE:</label> <span>${complaint.message || 'N/A'}</span><br>
                        <label>DATE DECLINED:</label> <span>${complaint.updated_at || complaint.created_at || 'N/A'}</span>
                    `;
        ...
        function openComplaintModal(idx) {
            ...
            modalContent.innerHTML = `
                <div class="modal-content">
                    ...
                    <label>COMPLAINT NUMBER:</label> <span>${complaint.complaintID}</span><br>
                    <label>TYPE:</label> <span>${complaint.type ? complaint.type.toUpperCase() : 'N/A'}</span><br>
        ...
        async function updateComplaintStatus(complaintId, status) {
            ...
            try {
                ...
                if (response.ok) {
                    closeModal();
                    // Remove the complaint from the complaints array and re-render
                    complaints = complaints.filter(c => {
                        // Use the correct complaint ID field for comparison
        ...
        setInterval(fetchApprovedComplaints, 5000);
        setInterval(fetchDeclinedComplaints, 5000);
